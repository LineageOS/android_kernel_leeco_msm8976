...
show_prototype(boe_short_circuit_test)
show_prototype(boe_open_circuit_test)
show_prototype(boe_raw_cap_data)
show_prototype(truly_open_circuit_test)
show_prototype(calibration_data_freq0)
show_prototype(calibration_data_freq1)
show_prototype(calibration_data_freq2)
show_prototype(calibration_data_is)
...
static struct attribute *attrs[] = {
	...
	attrify(no_auto_cal),
	attrify(read_report),
	attrify(boe_short_circuit_test),
	attrify(boe_open_circuit_test),
	attrify(boe_raw_cap_data),
	attrify(truly_open_circuit_test),
	attrify(calibration_data_freq0),
	attrify(calibration_data_freq1),
	attrify(calibration_data_freq2),
	attrify(calibration_data_is),
	attrify(calibration_data_test),
	NULL,
};
ssize_t (*s3320_boe_raw_cap_data_show)(struct device *dev, struct device_attribute *attr, char *buf) = test_sysfs_boe_raw_cap_data_show;
ssize_t (*s3320_boe_open_circuit_test_show)(struct device *dev, struct device_attribute *attr, char *buf) = test_sysfs_boe_open_circuit_test_show;
ssize_t (*s3320_boe_short_circuit_test_show)(struct device *dev, struct device_attribute *attr, char *buf) = test_sysfs_boe_short_circuit_test_show;
ssize_t (*s3320_truly_open_circuit_test_show)(struct device *dev, struct device_attribute *attr, char *buf) = test_sysfs_truly_open_circuit_test_show;

ssize_t (*synaptics_calibration_data_freq0_show)(struct device *dev, struct device_attribute *attr, char *buf) = test_sysfs_calibration_data_freq0_show;

ssize_t (*synaptics_calibration_data_freq1_show)(struct device *dev, struct device_attribute *attr, char *buf) = test_sysfs_calibration_data_freq1_show;

ssize_t (*synaptics_calibration_data_freq2_show)(struct device *dev, struct device_attribute *attr, char *buf) = test_sysfs_calibration_data_freq2_show;

ssize_t (*synaptics_calibration_data_is_show)(struct device *dev, struct device_attribute *attr, char *buf) = test_sysfs_calibration_data_is_show;

ssize_t (*synaptics_calibration_data_test_show)(struct device *dev, struct device_attribute *attr, char *buf) = test_sysfs_calibration_data_test_show;
...
static int test_do_afe_calibration(enum f54_afe_cal mode)
{
	...
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	printk("Synaptics Enter test_do_afe_calibration mode:%d\n",mode);
	retval = synaptics_rmi4_reg_read(rmi4_data,
			f54->control.reg_188->address,
			f54->control.reg_188->data,
			sizeof(f54->control.reg_188->data));
	...
	if (mode == F54_AFE_CAL)  // 0
		f54->control.reg_188->start_calibration = 1;
	else if (mode == F54_AFE_IS_CAL)  // 1
...
static ssize_t test_sysfs_calibration_data_freq0_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	//retval = synaptics_rmi4_f54_read_report(dev, attr, report_type, sizeof(report_type));
	...
	cnt += snprintf(buf + cnt, 4*PAGE_SIZE - cnt, "Collect data for freq: 0\n");

	cnt += snprintf(buf + cnt, 4*PAGE_SIZE - cnt, "tx = %d ,rx = %d\n",tx_num,rx_num);

	cnt += snprintf(buf + cnt, 4*PAGE_SIZE - cnt, "Calibration Image - Coarse & Fine\n");
	report_data_8 = head;
	...
	for (ii = 0; ii < rx_num; ii++)
	{
		for (jj = 0; jj < tx_num; jj++)
        	{
			cnt += snprintf(buf + cnt,4* PAGE_SIZE - cnt, "%02x ",
					*report_data_8);

			report_data_8 +=2;

		}
		cnt += snprintf(buf + cnt, 4* PAGE_SIZE - cnt, "\n");
	}
	...
	report_data_8 = (unsigned char*)f54->report_data;	//point to first byte of data
...
static ssize_t test_sysfs_calibration_data_freq1_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char report_type[10] = {0};
	char ii, jj;
	...
	cnt += snprintf(buf + cnt, 4* PAGE_SIZE - cnt, "Collect data for freq: 1\n");
	...
	cnt += snprintf(buf + cnt, 4* PAGE_SIZE - cnt, "\nCalibration Noise - Detail\n");
	report_data_8 = head;	//point to first byte of data
	report_data_8 += (tx_num * rx_num *2);
	for (ii = 0; ii < rx_num * 2 ; ii++)
	{
		cnt += snprintf(buf + cnt, 4* PAGE_SIZE - cnt, "%02x ",
...
static ssize_t test_sysfs_calibration_data_freq2_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char report_type[10] = {0};
	char ii, jj;
	...
	cnt += snprintf(buf + cnt, 4* PAGE_SIZE - cnt, "Collect data for freq: 2\n");
...
static ssize_t test_sysfs_calibration_data_is_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char report_type[10] = {0};
	char ii, jj;
	...
	head = (unsigned char*)f54->report_data + 2048;

	cnt += snprintf(buf + cnt, 4* PAGE_SIZE - cnt, "Collect data for interval scan\n");
...
/*	freq0 test  start */
	...
	for (ii = 0; ii < rx_num; ii++)
	{
		for (jj = 0; jj < tx_num; jj++)
    	    	{
				if(*report_data_8  > 0x07 ||*report_data_8 < 0x01)
	...
	*/
	...
	{
		if(ii == 0 || ii == 1){
			if(*report_data_8  > 0x3f ||*report_data_8 < 0x00)
                				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq0 button Rx:%d data:%x,Spec:0x00~0x3f\n",ii,*report_data_8);
			}
		if(ii == 2){
				if(*report_data_8  > 0x3f ||*report_data_8 < 0x00)
                				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq0 button Rx:%d data:%x,Spec:0x00~0x3f\n",ii,*report_data_8);
		...
		if(ii == 3){
				if(*report_data_8  > 0xf0 ||*report_data_8 < 0x50)
                				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq0 button Rx:%d data:%x,Spec:0x50~0xf0\n",ii,*report_data_8);
		}
		report_data_8 +=2;
	}
/*	freq0 test  end */
...
/*	freq1 test  start */
	...
	for (ii = 0; ii < rx_num; ii++)
	{
		for (jj = 0; jj < tx_num; jj++)
      	 	{
		if(*report_data_8  > 0x07 ||*report_data_8 < 0x01)
                    		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq1 image Tx:%d Rx:%d data:%x,Spec:0x01~0x07\n",
                            ii, jj, *report_data_8);
			report_data_8 +=2;
	...
	*/
	...
	for (ii = 0; ii < 4; ii++)
	{
			if(ii == 0 || ii == 1){
				if(*report_data_8  > 0x3f ||*report_data_8 < 0x00)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq1 button Rx:%d data:%x,Spec:0x00~0x3f\n",ii,*report_data_8);
				}
			if(ii == 2){
					if(*report_data_8  > 0x3f ||*report_data_8 < 0x00)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq1 button Rx:%d data:%x,Spec:0x00~0x3f\n",ii,*report_data_8);
			...
			if(ii == 3){
					if(*report_data_8  > 0xf0 ||*report_data_8 < 0x50)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq1 button Rx:%d data:%x,Spec:0x50~0xf0\n",ii,*report_data_8);
			}
		report_data_8 +=2;
	}
/*	freq1 test  end */
...
/*	freq2 test  start */
	...
	for (ii = 0; ii < rx_num; ii++)
	{
		for (jj = 0; jj < tx_num; jj++)
      	  	{
			if(*report_data_8  > 0x07 ||*report_data_8 < 0x01)
                    		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq2 image Tx:%d Rx:%d data:%x,Spec:0x01~0x07\n",
                            ii, jj, *report_data_8);
			report_data_8 +=2;
		}
	}
	...
	*/
	...
	for (ii = 0; ii < 4; ii++)
	{
		if(ii == 0 || ii == 1){
				if(*report_data_8  > 0x3f ||*report_data_8 < 0x00)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq2 button Rx:%d data:%x,Spec:0x00~0x3f\n",ii,*report_data_8);
				}
			if(ii == 2){
					if(*report_data_8  > 0x3f ||*report_data_8 < 0x00)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq2 button Rx:%d data:%x,Spec:0x00~0x3f\n",ii,*report_data_8);
			...
			if(ii == 3){
					if(*report_data_8  > 0xf0 ||*report_data_8 < 0x50)
                    				cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq2 button Rx:%d data:%x,Spec:0x50~0xf0\n",ii,*report_data_8);
			}
		report_data_8 +=2;
	}
/*	freq2 test  end */
...
static ssize_t test_sysfs_boe_short_circuit_test_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char report_type[10] = { 0};
	int ii;
...
*/
	if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "
	for (ii = 0; ii < tx_num; ii++) {
		for (jj = 0; jj < rx_num; jj++) {
#if 0
			if( !((ii == tx_num - 1) &&( jj  >  2)))
			{
                		if( *report_data_16 > open_circuit_threshold_boe[ii][jj]*(100+25)/100 ||
                     	  	 *report_data_16 < open_circuit_threshold_boe[ii][jj]*(100-25)/100 )
                    		return snprintf(buf, PAGE_SIZE, "TP Open Circuit Detected,\nTx:%d,Rx:%d,raw:%d,threshold:%d\n",
                            	ii, jj, *report_data_16, open_circuit_threshold_boe[ii][jj]);
			report_data_16++;
			}
#else
			if( *report_data_16 > open_circuit_threshold_boe[ii][jj]*(100+30)/100 ||
                                *report_data_16 < open_circuit_threshold_boe[ii][jj]*(100-30)/100 )
                    		return snprintf(buf, PAGE_SIZE, "TP Open Circuit Detected,\nTx:%d,Rx:%d,raw:%d,threshold:%d\n",
                            	ii, jj, *report_data_16, open_circuit_threshold_boe[ii][jj]);
			report_data_16++;
#endif
...
#if 0
		if(ii < tx_num - 2)
			report_data_16 = report_data_16 + 3;
		else
			{
				if(ii == tx_num-2)
					report_data_16 = report_data_16 +f54->rx_assigned ;
			}
#else
		report_data_16 = report_data_16 + 3;
#endif
...
static ssize_t test_sysfs_truly_open_circuit_test_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	//test_do_afe_calibration(F54_AFE_CAL);
	...
	report_data_16_result = report_data_u16;
	//VA area 205~305   255+/-50
	for(ii = 0; ii < 540; ii++)
	{
		if(*report_data_16_result < 205 || *report_data_16_result > 305)
		{
			result = 0;
			break;
		}
		report_data_16_result++;
	}
	//virtual key 155~355   255+/-100
	for(ii = 540; ii < 543; ii++)
	{
		if(*report_data_16_result < 155 || *report_data_16_result > 355)
		{
			result = 0;
			break;
		}
		//printk("synaptics key %d\n",*report_data_16_result);
		report_data_16_result++;
	}
	if(result)
	{
		cnt = snprintf(buf, PAGE_SIZE, "Pass\n");
		buf += cnt;
		count += cnt;
	}
	else
	{
		cnt = snprintf(buf, PAGE_SIZE, "Fail\n");
		buf += cnt;
		count += cnt;
	}
...
static ssize_t test_sysfs_boe_raw_cap_data_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char report_type[10] = {0};
	int ii;
	...
	if(!strcmp(f54->rmi4_data->rmi4_mod_info.product_id_string, "S2-05-01"))
	{
...
unsigned long cal_arg = 0;

static ssize_t calibration_show(struct kobject *kobj, struct kobj_attribute *attr,
			  char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%ld\n",cal_arg);
}

static ssize_t calibration_store(struct kobject *kobj, struct kobj_attribute *attr,
			   const char *buf, size_t n)
{
	int retval;
	//unsigned long setting;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	retval = sstrtoul(buf, 10, &cal_arg);
	if (retval)
		return retval;
	printk("Synaptics Enter calibration_store setting:%ld\n",cal_arg);
	if (!f54->query_49.has_ctrl188) {
		dev_err(rmi4_data->pdev->dev.parent,
...
tpswitch_ctrl_attr(calibration);
...
{
	tpswitch_ctrl_kobj = kobject_create_and_add("tp_calibration", NULL);
		...
		struct device_attribute *attr, char *buf)
{
	...
	case F54_8BIT_IMAGE:
		...
		break;
	case F54_AMP_RAW_ADC:   //truly type:83
		report_data_u16 = (unsigned short *)f54->report_data;
		report_data_16_result = report_data_u16;
		for(ii = 0; ii < 543; ii++)
		{
	...
	case F54_TRUE_BASELINE:
	case F54_FULL_RAW_CAP:
	case F54_FULL_RAW_CAP_NO_RX_COUPLING:   //BOE type:20
	case F54_SENSOR_SPEED:
	case F54_AMP_FULL_RAW_CAP:
		report_data_16 = (short *)f54->report_data;
		cnt = snprintf(buf, PAGE_SIZE - count, "tx = %d\nrx = %d\n",
