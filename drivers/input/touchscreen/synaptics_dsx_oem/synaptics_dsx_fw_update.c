...
#define FW_IMAGE_NAME_BOE "synaptics/startup_boe_fw_update_05005706.img"
#define FW_IMAGE_NAME_TRULY "synaptics/startup_truly_fw_update_03005708.img"
...
#define PID_BOE_S3320 "S2-05-01"
#define PID_TRULY_S332U "S332U"
...
#ifdef CONFIG_GET_HARDWARE_INFO
static void synaptics_hardware_info_reg(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	char cfg[10];
	...
	if(0 == strcasecmp(product_id, PID_BOE_S3320))  //TP IC:S3320A;panel:BOE
	{
		strcpy(tmp_tp_name, "S3320-BOE-cfg:");
	    sprintf(cfg,"%02x",config_id[3]);
	    strcat(tmp_tp_name,cfg);
	}
	else if(0 == strcasecmp(product_id, PID_TRULY_S332U))  //TP IC:S332U;panel:truly
	{
	    strcpy(tmp_tp_name, "S332U-TRULY-cfg:");
	    sprintf(cfg,"%02x",config_id[3]);
	    strcat(tmp_tp_name,cfg);
	}
...
static enum flash_area fwu_go_nogo(void)
{
	...
	device_config_id = extract_uint_be(fwu->config_id);
	dev_info(rmi4_data->pdev->dev.parent,
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		...
		SYNAP_INFO("%s product_id:%s",__func__,product_id);

		if(0 == strcasecmp(product_id, PID_BOE_S3320))
		{
			SYNAP_INFO("%s copy BOE",__func__);
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				FW_IMAGE_NAME_BOE, sizeof(FW_IMAGE_NAME_BOE),
				sizeof(FW_IMAGE_NAME_BOE));
		}
		else if(0 == strcasecmp(product_id, PID_TRULY_S332U))
		{
			SYNAP_INFO("%s copy TRULY",__func__);
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
	...
	case UI_CONFIG:
		...
		if (retval < 0)
			break;
		retval = fwu_write_ui_configuration();
		SYNAP_INFO("Enter %s before reset_device:true",__func__);
		rmi4_data->reset_device(rmi4_data, true);
		break;
	case NONE:
	default:
...
#ifdef CONFIG_GET_HARDWARE_INFO
	synaptics_hardware_info_reg(rmi4_data);
#endif
